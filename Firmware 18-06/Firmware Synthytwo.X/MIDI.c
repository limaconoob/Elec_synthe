...
void        calculatePitch(void)
{
    ...
    pitch1 = tab_period[midi_note] / x;
    pitch_adjust1 = ((tab_period[midi_note] % x) * 100) / x;
    phi1 = pitch1 - 1;

    if (midi_note > 4) {
        pitch2 = tab_period[midi_note - 4] / x;
        pitch_adjust2 = ((tab_period[midi_note - 4] % x) * 100) / x;
    }
    else {
        pitch2 = tab_period[0] / x;
        pitch_adjust2 = ((tab_period[0] % x) * 100) / x;
    }
    phi2 = pitch2 - 1;
}
