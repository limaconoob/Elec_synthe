...
u16     generateAudioSample(void)
{
    ...
    if (osc1_sample > 0x7FFF && osc2_sample > 0x7FFF)
        mix_sample = 0xFFFF - ((0xFFFF - osc1_sample) + (0xFFFF - osc2_sample));
    ...
    else
    {
        if (osc2_sample < 0xFFFF - osc1_sample)
            ...
            mix_sample = osc2_sample - (0xFFFF - osc1_sample);
    }

/*
    ---------------  NOISE -------------------
    if (noise_sample > 0x7FFF && mix_sample > 0x7FFF)
        mix_sample = 0xFFFF - ((0xFFFF - noise_sample) + (0xFFFF - mix_sample));
    else if (noise_sample <= 0x7FFF && mix_sample <= 0x7FFF)
        mix_sample = (noise_sample + mix_sample);
    else if (noise_sample <= 0x7FFF)
    {
        if (noise_sample < 0xFFFF - mix_sample)
            mix_sample = 0xFFFF - ((0xFFFF - mix_sample) + noise_sample);
        else
            mix_sample = noise_sample + (0xFFFF - mix_sample);
    }
    else
    {
        if (mix_sample < 0xFFFF - noise_sample)
            mix_sample = 0xFFFF - ((0xFFFF - noise_sample) + mix_sample);
        else
            mix_sample = mix_sample + (0xFFFF - noise_sample);
    }
*/

    // VOLUME
  //  mix_sample = (mix_sample / 16) * cur_preset.MIX_Glide;
    if (!mix_sample)
        mix_sample = 1;
    return (mix_sample);
}
